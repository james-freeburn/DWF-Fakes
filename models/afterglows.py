...
def GRB_parameters(rng,nevents):
    ...
    b_vals = rng.uniform(0.0,3.0,nevents)
    
    # Drawing afterglows equally from different jet types.
    jet_types = [0,1,2,4]
    jet = rng.choice(jet_types,size=nevents)
    
    # b is not used for Gaussian jet structures.
    b_vals = [-1.0 if (jet[i] == 0) or (jet[i] == 2) else b_vals[i] 
              for i in range(nevents)]
    ...
    T90 = np.exp(stats.truncnorm(
            (lower - mu3) / sigma3, (upper - mu3) / sigma3, loc=mu3, 
            scale=sigma3).rvs(nevents))
    ...
    # GRB Spectrum
    Es = np.geomspace(1.0,1e4,int(1e5)) #keV
